...
| Management fee wording | `plans.manager` says net rental income. The management page's pricing card says "of nightly-rental revenue" and "Calculated on nightly rate (plus any extra-guest or premium-stay fees)". |
...
- Root layout: no root-level canonical, so nested pages cannot inherit the homepage URL; every public page declares its own. Titles on the six pages that doubled the suffix now use `title.absolute`. Root default title and description updated.
...
3. **Management fee-base wording left untouched.** `plans.manager` says net rental income and most of the site agrees. The management page's own pricing card says nightly-rental revenue. That contradiction is on the same page and is real, but the brief says not to reinterpret the business agreement, so it is a release gate rather than a code change.
